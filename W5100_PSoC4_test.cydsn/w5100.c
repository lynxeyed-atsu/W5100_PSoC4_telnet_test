 ...
 *							: Change control type for SPI port from uint8 to bit.
 * ----------	-------		------------------------------------------------
...
Maximum memory size for Tx, Rx in the W5100 is 8K uint8s,\n
In the range of 8Kuint8s, the memory size could be allocated dynamically by each channel.\n
Be attentive to sum of memory size shouldn't exceed 8Kuint8s\n
and to data transmission and receiption from non-allocated channel may cause some problems.\n
If the 8Kuint8s memory is already  assigned to centain channel, \n
other 3 channels couldn't be used, for there's no available memory.\n
If two 4Kuint8s memory are assigned to two each channels, \n
other 2 channels couldn't be used, for there's no available memory.\n
*/ 
void sysinit(
	uint8 tx_size, 	/**< tx_size Tx memory size (00 - 1Kuint8, 01- 2KBtye, 10 - 4Kuint8, 11 - 8Kuint8) */
	uint8 rx_size		/**< rx_size Rx memory size (00 - 1Kuint8, 01- 2KBtye, 10 - 4Kuint8, 11 - 8Kuint8) */
	)
{
	int16 i;
	int16 ssum,rsum;
...
*/ 
void setGAR(
	uint8 * addr	/**< a pointer to a 4 -uint8 array responsible to set the Gateway IP address. */
	)
{
...
*/ 
void setSUBR(
	uint8 * addr	/**< a pointer to a 4 -uint8 array responsible to set the SubnetMask address */
	)
{
...
*/ 
void setSHAR(
	uint8 * addr	/**< a pointer to a 6 -uint8 array responsible to set the MAC address. */
	)
{
...
*/
void setSIPR(
	uint8 * addr	/**< a pointer to a 4 -uint8 array responsible to set the Source IP address. */
	)
{
...
register. User should read upper uint8 first and lower uint8 later to get proper value.
*/
...
User should read upper uint8 first and lower uint8 later to get proper value.
*/
...
uint8 pppinit_in(uint8 * id, uint8 idlen, uint8 * passwd, uint8 passwdlen)
{
   ...
   /*---*/
			...
			if ( len > 0 )
			{
				...
				len += 2;
				str_idx = 6; dst_idx = 0; // ppp header is 6 uint8, so starts at 6.
				do 
				{
